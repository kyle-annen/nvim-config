...
- [x] **Wire up `on_attach` and `capabilities`** — Exported from `lsp-configs.lua` as a module return table and wired into the mason-lspconfig handler in `plugins.lua`. Also fixed deprecated `vim.lsp.buf.formatting()` → `vim.lsp.buf.format()`.
