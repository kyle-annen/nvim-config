...
- [ ] **Switch from Solargraph to ruby-lsp** — Solargraph is slow, resource-heavy, and struggles with large Rails projects. Shopify's `ruby-lsp` is significantly faster, uses less memory, and has better Rails support. Replace `solargraph` with `ruby_ls` in mason-lspconfig `ensure_installed`.
